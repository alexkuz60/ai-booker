...
- [ ] **Хеш-верификация контента при чтении из OPFS** — при записи `scenes/chapter_*.json` считать FNV-1a хеш каждого текстового блока и хранить рядом (`_hash`); при чтении пересчитывать и сравнивать. Несовпадение → пометить сцену как повреждённую, показать предупреждение пользователю. Серверный аналог уже работает в `synthesize-scene` (TTS-кеш)
