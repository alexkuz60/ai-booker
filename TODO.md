...
- [x] **Хеш-верификация контента при чтении из OPFS** — FNV-1a 32-bit хеш в `scene_index.json.contentHash`, сравнение с `storyboard.json.contentHash` при открытии раскадровки. Реализовано в `src/lib/contentHash.ts` + `src/lib/sceneIndex.ts`. **Фикс B18:** `StoryboardSnapshot` теперь включает `contentHash`, предотвращая потерю хеша при ручных правках раскадровки
