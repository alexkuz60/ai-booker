...
- [x] **Авто-синхронизация перевода при Push** — на странице Translation кнопка «На сервер» автоматически сохраняет и основной проект, и проект-зеркало перевода. Отдельная кнопка ручного пуша перевода удалена
- [x] **Восстановление перевода с сервера** — кнопка «Перевод ← сервер» загружает translation-зеркало из облака (ZIP-импорт)
...
- [x] **Сохранение метаданных при Push** — spread `...freshMeta` в `useSaveBookToProject` гарантирует сохранение `pipelineProgress`, `translationProject` и др. полей при записи `project.json` (B28)
