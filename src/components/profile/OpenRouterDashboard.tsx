...
import { Loader2, Wifi, Zap, AlertTriangle, Key, Globe, Save, RefreshCw, Wallet } from 'lucide-react';
...
export function OpenRouterDashboard({ hasKey, apiKeyValue, onApiKeyChange, onSave, saving, language }: OpenRouterDashboardProps) {
    ...
    <Accordion type="multiple" defaultValue={['apikey']} className="mb-4">
      <AccordionItem value="apikey" className="border rounded-lg px-4">
        ...
        <AccordionContent className="pb-4">
          <ApiKeyField provider="openrouter" label="OpenRouter API Key" value={apiKeyValue} onChange={onApiKeyChange} placeholder="sk-or-..."
            hint={<>{isRu ? 'Получите ключ на' : 'Get your key at'}{' '}<a href="https://openrouter.ai/keys" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">openrouter.ai/keys</a></>}
          />
        </AccordionContent>
      </AccordionItem>
    </Accordion>
  ...
  const renderInfoBlock = () => (
    <div className="flex items-center gap-3 mb-4 p-3 rounded-lg bg-primary/5 border border-primary/20">
      ...
      <div className="flex-1">
        <p className="text-sm font-semibold text-primary mb-1">{isRu ? 'Универсальный AI-шлюз' : 'Universal AI Gateway'}</p>
        <p className="text-xs text-muted-foreground">{isRu ? 'Единый шлюз к множеству моделей через один API-ключ.' : 'Unified gateway to many models via one API key.'}</p>
      </div>
    </div>
  );
  ...
  if (!hasKey) {
    return (
      <Card className="border-border/50 bg-card/50 backdrop-blur-sm p-6">
        <CardHeader className="flex flex-row items-center gap-2 pb-4"><Globe className="h-5 w-5 text-primary" /><CardTitle className="font-display">OpenRouter</CardTitle></CardHeader>
        <CardContent>
          {renderInfoBlock()}
          {renderKeySection()}
          <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/50 border border-border">
            <AlertTriangle className="h-5 w-5 text-amber-500 flex-shrink-0" />
            <p className="text-sm text-muted-foreground">{isRu ? 'Добавьте ключ OpenRouter и сохраните.' : 'Add your OpenRouter key and save.'}</p>
          </div>
          <div className="flex justify-end pt-2">
            <Button onClick={onSave} disabled={saving} size="sm">
              {saving ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Save className="h-4 w-4 mr-2" />}{isRu ? 'Сохранить' : 'Save'}
            </Button>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <Card className="border-border/50 bg-card/50 backdrop-blur-sm p-6">
      <CardHeader className="flex flex-row items-center gap-2 pb-4"><Globe className="h-5 w-5 text-primary" /><CardTitle className="font-display">OpenRouter Dashboard</CardTitle></CardHeader>
      <CardContent>
        {renderInfoBlock()}
        {renderKeySection()}
          ...
          <AccordionItem value="status" className="border rounded-lg px-4">
            ...
            <AccordionContent className="space-y-4 pb-4">
              <div className="flex items-center gap-3">
                <Button onClick={api.handlePing} disabled={api.pinging} size="sm">
                  {api.pinging ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Zap className="h-4 w-4 mr-2" />}Ping
                </Button>
                {api.pingResult && (
                  <span className="text-sm text-muted-foreground">{isRu ? 'Латенси' : 'Latency'}: <strong className="text-foreground">{api.pingResult.latency_ms}ms</strong></span>
                )}
              </div>
              {api.keyInfo && (
                <div className="rounded-lg border bg-muted/30 p-3 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" className="h-6 w-6 ml-auto" onClick={api.fetchKeyInfo} disabled={api.keyLoading}>
                      <RefreshCw className={cn("h-3.5 w-3.5", api.keyLoading && "animate-spin")} />
                    </Button>
                  </div>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                    {api.keyInfo.label && <><span className="text-muted-foreground">{isRu ? 'Метка' : 'Label'}:</span><span className="font-mono text-right">{api.keyInfo.label}</span></>}
                    <span className="text-muted-foreground">{isRu ? 'Сегодня' : 'Today'}:</span><span className="font-mono text-right">${api.keyInfo.usage_daily?.toFixed(4) ?? '0'}</span>
                    <span className="text-muted-foreground">{isRu ? 'Месяц' : 'Month'}:</span><span className="font-mono text-right">${api.keyInfo.usage_monthly?.toFixed(4) ?? '0'}</span>
                    {api.keyInfo.limit !== null && <>
                      <span className="text-muted-foreground">{isRu ? 'Лимит' : 'Limit'}:</span><span className="font-mono text-right">${api.keyInfo.limit?.toFixed(2)}</span>
                      <span className="text-muted-foreground">{isRu ? 'Остаток' : 'Remaining'}:</span><span className="font-mono text-right font-medium text-primary">${api.keyInfo.limit_remaining?.toFixed(4)}</span>
                    </>}
                  </div>
                </div>
              )}
