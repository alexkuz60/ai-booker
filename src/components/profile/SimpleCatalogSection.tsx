...
interface SimpleCatalogSectionProps {
  userAddedModels: ProxyApiCatalogModel[];
  filteredCatalogModels: { id: string; name?: string; owned_by?: string }[];
...
export function SimpleCatalogSection({
  userAddedModels, filteredCatalogModels, userModelIds,
      ...
      <AccordionContent className="pb-4 space-y-3">
        <div className="relative z-10 flex items-center gap-2 pt-1 px-0.5">
          <div className="relative flex-1">
            ...
            <Input value={catalogSearch} onChange={e => onCatalogSearchChange(e.target.value)}
              placeholder={catalogLoading ? (isRu ? 'Загрузка каталога...' : 'Loading catalog...') : (isRu ? `Поиск среди ${catalogCount} моделей ${providerName}...` : `Search ${catalogCount} ${providerName} models...`)}
              className="pl-9 focus-visible:ring-offset-0" disabled={catalogLoading}
            />
            ...
            <div className="border rounded-lg bg-card/50 max-h-[240px] overflow-y-auto">
              {filteredCatalogModels.map(model => (
                <label key={model.id} className="flex items-center gap-2 px-3 py-2 hover:bg-muted/50 cursor-pointer border-b border-border/30 last:border-0">
                  ...
                  <span className="text-xs text-muted-foreground truncate max-w-[120px]">{(model as any).name || (model as any).owned_by}</span>
                </label>
              ))}
            </div>
            {selectedIds.size > 0 && (
              <Button size="sm" onClick={handleBulkAdd} className="gap-2"><Plus className="h-4 w-4" />{isRu ? 'Добавить' : 'Add'} ({selectedIds.size})</Button>
            )}
          </div>
        )}
        {catalogSearch.trim() && filteredCatalogModels.length === 0 && !catalogLoading && (
          <p className="text-xs text-muted-foreground text-center py-2">{isRu ? `Ничего не найдено среди ${catalogCount} моделей` : `No results among ${catalogCount} models`}</p>
        )}
          ...
          <Collapsible open={userListOpen} onOpenChange={setUserListOpen}>
            ...
            <CollapsibleContent className="space-y-1 mt-1">
              {userAddedModels.map(model => (
                <div key={model.id} className="flex items-center gap-3 p-3 rounded-lg border border-primary/20 bg-accent/30 hover:bg-accent/50 transition-colors">
                  ...
                  {testResults[model.id] && <TestStatusIcon testResult={testResults[model.id]} lang={lang} />}
                  <Button size="sm" variant="ghost" className="flex-shrink-0 h-8 w-8 p-0" onClick={() => onTestModel(model.id)} disabled={testingModel === model.id}>
                    {testingModel === model.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}
                  </Button>
                  <Button size="sm" variant="ghost" className="flex-shrink-0 h-8 w-8 p-0 text-muted-foreground hover:text-destructive" onClick={() => onRemoveUserModel(model.id)}>
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <Tooltip>
        ...
        <TooltipContent side="top" className="max-w-[280px]"><p className="text-xs">{testResult.message || expl?.description || testResult.error}</p></TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
}
