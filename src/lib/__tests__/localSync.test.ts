...
/** Minimal in-memory mock of ProjectStorage (V2 layout) */
function createMockStorage(): ProjectStorage & { _data: Record<string, unknown> } {
  const data: Record<string, unknown> = {
    "project.json": { version: 1, bookId: "b1", title: "Test", userId: "u1", createdAt: "", updatedAt: "", language: "en", layoutVersion: 2 },
  };
...
describe("localSync roundtrip (V2)", () => {
  ...
  it("skips folder nodes (level 0 with children at level 1)", async () => {
    ...
    // Folder node (index 0) should not have content file
    expect(storage._data["chapters/part-1/content.json"]).toBeUndefined();
  });
});
