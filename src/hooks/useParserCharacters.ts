...
export function useParserCharacters({
  ...
  // Profile selected characters using AI
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("profile-characters-local", {
        body: {
          ...
          scenes: scenesPayload.slice(0, 30), // Limit context size
          lang: isRu ? "ru" : "en",
          model: profilerModel,
          apiKey: apiKeyForModel,
  ...
  }, [characters, chapterResults, tocEntries, profilerModel, userApiKeys, isRu, persist, toast]);
