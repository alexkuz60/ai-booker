...
import {
  ...
  DEFAULT_SETTINGS, SETTINGS_KEY, USER_MODELS_KEY,
...
export function useProxyApiData(hasKey: boolean) {
  ...
  const [goneModel, setGoneModel] = useState<string | null>(null);
  ...
  const proxyModels: { id: string; name: string }[] = [];
  ...
  const fetchCatalog = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (data.models) { setProxyCatalog(data.models as ProxyApiCatalogModel[]); setCatalogLoaded(true); }
  ...
  const handleTestModel = useCallback(async (modelId: string) => {
    ...
    try {
      ...
      if (data.status === 'gone') setGoneModel(modelId);
      fetchLogs();
    } catch {
  ...
  const handleExportCSV = useCallback(() => {
    ...
    a.href = url; a.download = `proxyapi-logs-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
