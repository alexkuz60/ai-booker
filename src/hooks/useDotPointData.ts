...
import {
  type ProxyApiCatalogModel, type PingResult, type TestResult,
  type LogEntry, type ProxyApiSettings, type AnalyticsEntry,
  DEFAULT_SETTINGS,
...
export function useDotPointData(hasKey: boolean) {
  ...
  const { value: cloudSettings, update: updateCloudSettings, loaded: settingsLoaded } = useCloudSettings<ProxyApiSettings>(
    'dotpoint-settings', DEFAULT_SETTINGS, 'dotpoint_settings',
  );
  const { value: cloudUserModels, update: updateCloudUserModels } = useCloudSettings<string[]>(
    'dotpoint-user-models', [], 'dotpoint_user_models',
  );
  ...
  const dotpointModels: { id: string; name: string }[] = [];
  ...
  const fetchCatalog = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (data.models) { setDotpointCatalog(data.models as ProxyApiCatalogModel[]); setCatalogLoaded(true); }
    } catch { /* silent */ } finally { setCatalogLoading(false); }
  }, [user, catalogLoaded]);
  ...
  const handleMassTest = useCallback(async () => {
    ...
    const allModels = [...dotpointModels.map(m => m.id), ...userAddedModels.map(m => m.id)];
  ...
  const deleteModelStats = useCallback(async (rawModelId: string, displayModel?: string) => {
    if (user) {
      try { await supabase.from('proxy_api_logs').delete().eq('user_id', user.id).eq('model_id', rawModelId); } catch { /* silent */ }
    }
    const target = displayModel || rawModelId;
    updateCloudUserModels(prev => prev.filter(id => id !== target));
  ...
  const handleExportCSV = useCallback(() => {
    ...
    a.href = url; a.download = `dotpoint-logs-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
    URL.revokeObjectURL(url);
  }, [logs]);

  useEffect(() => { if (hasKey && user && !catalogLoaded) fetchCatalog(); }, [hasKey, user, catalogLoaded, fetchCatalog]);
  useEffect(() => { if (hasKey && user) fetchLogs(); }, [hasKey, user, fetchLogs, logsRefreshTrigger]);
