...
│                   ⚠ Аудио-файлы (audio_meta, mixer_state, clip_plugins, audio/tts)
│                     НЕ хранятся в {lang}/. Озвучка перевода реализуется через
│                     экспорт перевода в новый полноценный проект книги.
...
### 1.5a Жизненный цикл файлов: создание vs редактирование

> **Контракт: во время работы в Студии (runtime) ничего не создаётся — только редактируется.**

Все JSON-файлы и папки создаются **ТОЛЬКО** в двух точках:
1. **`syncStructureToLocal()`** → `seedEmptySceneFiles()` — при первом анализе книги (после создания проекта)
2. **`OPFSStorage.createNewProject()`** → корневые файлы и папки по шаблону `bookTemplateOPFS.ts`

В runtime (Студия, Раскадровка, Микшер, ТТС-рендер, Монтаж) код:
- **✅ Читает** существующие файлы по путям из `bookMap`
- **✅ Редактирует** содержимое существующих JSON-файлов (добавляет/обновляет записи)
- **✅ Записывает** аудио-блобы в существующие папки (`tts/`, `audio/atmosphere/`)
- **❌ НЕ создаёт** новые JSON-файлы (audio_meta, mixer_state, clip_plugins — должны существовать)
- **❌ НЕ создаёт** новые папки

Если файл не найден при чтении — это ошибка инициализации, а не повод создать файл на лету. Логируется предупреждение, операция пропускается.

**Дефолтные значения** (пустой `{}`, пустой `entries: {}`, и т.д.) прописываются заранее в `bookTemplateOPFS.ts` и записываются в файлы при `seedEmptySceneFiles()`.

**Почему это важно:**
- Ленивое создание файлов может **перезаписать** уже существующие данные при race condition
- Невозможно отличить «файл ещё не создан» от «файл удалён/повреждён»
- Гарантирует, что `readStructureFromLocal()` — чисто read-only операция
...
- При нажатии «На сервер» данные перевода (JSON-файлы из lang-поддиректорий + папка `synopsis/`) упаковываются в ZIP и загружаются в `book-uploads/{userId}/translation_{bookId}.zip`. Аудио-файлы перевода не хранятся в lang-поддиректориях — озвучка реализуется через экспорт перевода в отдельный проект.
...
- `TranslationProjectLink` — интерфейс ссылки на зеркало (удалён из `ProjectMeta`)
- `sourceProjectName` / `targetLanguage` — deprecated поля зеркальной архитектуры (удалены из `ProjectMeta`)

**Утилита миграции (удалена):**
- `migrateMirrorTranslation.ts` — была удалена вместе с зеркальной архитектурой. Архитектурный тест запрещает любые ссылки.
...
3. ✅ Создать чистый OPFS-проект через `OPFSStorage.createNewProject()`
4. ✅ Записать ВСЕ данные с сервера (structure, characters, storyboards, scene_index, audio_meta, clip_plugins, mixer_state, atmospheres, translations)
...
- голосовые конфигурации (`book_characters.voice_config`)
- аудио-метаданные (`segment_audio`)
...
**Edge Functions — клиент-центричная модель:** Все AI-функции (синтез TTS, генерация атмосферных промптов, коррекция ударений, детекция инлайновых нарративов, сегментация сцен) получают данные в теле запроса от клиента (из OPFS), а НЕ читают из БД. Edge Functions **НЕ выполняют SELECT** к таблицам контента книги.
...
1. Кнопка «На сервер» (`saveBook` / `pushToDb`) — ручной экспорт пользователем
2. Wipe-and-Deploy — чтение из БД для полного восстановления проекта на новом устройстве
...
#### DNI-8. Зеркалирование профайлов персонажей

Данные (description, temperament и др.) обязаны присутствовать как в верхнеуровневых полях `CharacterIndex`, так и во вложенном объекте `profile` для обеспечения совместимости между БД и UI.

#### DNI-9. Единый проект — единый bookId

Один bookId = одна OPFS-папка. Переводы хранятся **внутри** проекта в `{lang}/` подпапках. Отдельные зеркальные OPFS-проекты **не существуют**. `ProjectMeta` содержит только `translationLanguages: string[]` для отслеживания активных локализаций.
...
- **OPFS** не даёт пользователю видеть файлы в проводнике ОС — это ограничение API, не баг. Для инспекции доступны: **OPFS Browser** (панель в Профиле, только для admin) и ZIP-экспорт.
...
### 1.17a OPFS Browser (admin-only)

Панель «OPFS Browser» в Профиле (вкладка «Хранилище», доступна только admin) — двухпанельный интерфейс для инспекции и управления OPFS-хранилищем:

- **Левая панель:** дерево навигации с глубиной до 6 уровней вложенности
- **Правая панель:** интерактивный просмотрщик JSON (`JsonTreeView`) с поддержкой сворачивания/разворачивания узлов
- **Гранулярное удаление:** для каждого файла или папки на любом уровне вложенности при наведении доступна иконка корзины
- Поддержка `ScrollArea` с адаптивной высотой

**Ключевые файлы:**

| Файл | Назначение |
|------|------------|
| `src/components/profile/tabs/OpfsBrowserPanel.tsx` | UI панели OPFS Browser |
| `src/components/profile/tabs/JsonTreeView.tsx` | Просмотрщик JSON с древовидной навигацией |
| `src/components/profile/tabs/StorageTab.tsx` | Вкладка «Хранилище» в Профиле |

### 1.18 Кэширование аудио-импульсов (IR) для реверберации
