...
4. Пользователь подтвердил → **полная замена** локального проекта в OPFS серверной копией (TOC, сцены, персонажи, метаданные). PDF скачивается из Supabase Storage если отсутствует локально.
5. Пользователь отказался → работа продолжается с локальными данными

**New Workstation Flow:** если книга есть на сервере, но не в OPFS (новое устройство / очищенный кэш) → автоматическая инициализация локальной структуры из БД без вопросов.
