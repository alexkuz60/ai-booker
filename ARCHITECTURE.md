...
#### DNI-9. Прямая связь проектов (Direct-link)

Ссылка на проект перевода (`projectName`) хранится исключительно в `project.json` основного проекта (поле `translationProject`). Использование эвристик, localStorage, файлов-ссылок (`_translation_link.json`) или внешних сканирований OPFS для поиска зеркал **запрещено**.
