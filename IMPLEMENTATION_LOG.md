...
| **3** | CharactersPanel с реестром из БД | ✅ | Загрузка по bookId, настройка голоса (табы Yandex/ElevenLabs), сохранение voice_config, кредиты ElevenLabs |
...
- **Что добавлено сверх плана:** `character_appearances`, инкрементальное извлечение, multi-provider voice_config, системные персонажи, автоочистка дубликатов, авто-кастинг, консистентность по алиасам

### 1.3. Модуль "Synthesis Pipeline" (Движок синтеза)

| Параметр | План СПРЗ | Реализация | Расхождение |
|----------|-----------|------------|-------------|
| **TTS-провайдер** | Yandex SpeechKit + ElevenLabs | Yandex SpeechKit (v1/v3), ElevenLabs (Multilingual v2) — двойной API | ✅ Совпадает |
...
- **Что реализовано сверх плана:** Кэширование, ре-синтез, инлайн-вставки, устаревание аудио, плейлисты, v1/v3 совместимость, смарт-разбивка, линковка first_person
...
### 1.5. Модуль "UX Shortcuts" (Горячие клавиши и визуальный фидбек)

| Параметр | План СПРЗ | Реализация | Расхождение |
|----------|-----------|------------|-------------|
| **Spacebar Play/Pause** | Не описан | Глобальный keydown-слушатель с защитой от INPUT/TEXTAREA/SELECT/contentEditable | 🆕 |
| **Иконка синтеза** | Не описана | AudioLines (волновая форма) вместо Play; пульсация (animate-pulse-glow) во время синтеза | 🆕 |
...
| UX Shortcuts | ✅ | Spacebar play/pause, AudioLines icon с пульсацией при синтезе |
