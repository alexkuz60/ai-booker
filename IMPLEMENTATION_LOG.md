...
- `WaveformEditor`: на **100% zoom** волна всегда отображает **полную длительность сцены** (включая стартовую тишину) на **всю ширину вьюпорта редактора**.
- Координаты волны, playhead, trim/fade и сегментных разделителей считаются в **scene-local** времени (`0..sceneDuration`).
- Запрещено сжимать/обрезать отображение по «active signal span»; масштабирование допускается только через zoom и scroll редактора.
