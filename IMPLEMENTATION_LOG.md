...
| **Системные персонажи** | Не описаны | «Рассказчик» и «Комментатор» — глобальные в `characters/index.json`, автопривязка через `upsertSpeakersFromSegments()` | 🆕 Не предусмотрено СПРЗ |
| **Консистентность** | Не описана | `buildNameLookup()` + `findCharacterByNameOrAlias()` — регистронезависимый поиск по имени/алиасам | 🆕 Предотвращает дублирование |
...
| **Пропагация спикеров** | Не описана | Только системные типы (narrator, epigraph, lyric, footnote). `inner_thought` и `first_person` исключены — спикер меняется индивидуально | 🆕 |
| **Автоназначение системного спикера** | Не описано | Смена типа → narrator/epigraph/lyric → спикер = «Рассказчик»; footnote → «Комментатор». Обратная смена (на dialogue/inner_thought) → очистка системного спикера | 🆕 |
| **Маршрутизация клипов** | Не описана | Системные типы (narrator, epigraph, lyric, footnote) ВСЕГДА маршрутизируются на трек системного персонажа, игнорируя typeMappings. Надпись клипа = тип блока (Рассказчик/Комментатор), а не поле speaker | 🆕 |
| **Массовые действия** | Не описаны | Мультивыбор сегментов + чекбокс «Все» в тулбаре; смена типа/спикера в любом выделенном → применяется ко всем выделенным, игнорируя пропагацию | 🆕 |
| **Очистка пустых треков** | Не описана | После пропагации типов — перестройка `characters/scene_{id}.json` → персонажи без сегментов удаляются | 🆕 |
| **Dirty-маркеры** | Не описаны | Явный `dirtyScenes[]` в scene_index. Устанавливается Парсером при изменении contentHash для сцен с раскадровкой (DNI-1). Сбрасывается Студией при переанализе. Не использует runtime-сравнение хешей | 🆕 |
...
## Модуль 2.5: Манипуляция атмосферными/SFX клипами (Timeline)

**Статус:** ✅ Реализовано (2026-03-26 — 2026-03-27)

### Функциональность

| Функция | Описание | Статус |
|---------|----------|--------|
| **Горизонтальное перемещение** | Drag клипа → обновление `offset_ms` в OPFS | ✅ |
| **Копирование/вставка** | Ctrl+C/V, вставка в позицию транспорта | ✅ |
| **Изменение скорости (ресайз)** | Drag за правый край → `speed` 0.5×–1.5× | ✅ |
| **Визуальный хинт скорости** | Жёлтый бейдж `×N.NN` у правого края при драге | ✅ |
| **Сброс скорости** | Контекстное меню → RotateCcw → speed=1, ширина восстанавливается | ✅ |
| **Визуальные направляющие** | Две жёлтые пунктирные линии: фиксированная + следящая за курсором | ✅ |
| **Лимит ширины при драге** | Визуальная ширина ограничена физическими лимитами 0.5×–1.5× | ✅ |
| **Fade-in/out** | Контекстное меню → пресеты (0.5–5 сек), персистентность в OPFS | ✅ |
| **Удаление клипа** | Контекстное меню → удаление + очистка настроек фейдинга | ✅ |
| **Insert audio** | Контекстное меню трека → выбор из Storage → вставка в указанную позицию | ✅ |

### Архитектура
- **Хук:** `useAtmoClipManipulation` — копирование, вставка, перемещение, ресайз, сброс скорости
- **Оптимистичное обновление:** `optimisticOffsets` и `optimisticResizes` в `TimelineTrack` для плавной визуализации без ожидания OPFS I/O
- **Персистентность:** `localAtmospheres.ts` — OPFS-only (CRUD: `addAtmosphereClip`, `updateAtmosphereClip`, `deleteAtmosphereClip`), fallback `readSceneIndex` при пустом кэше
- **Лупинг:** отключён на таймлайне (клип показывает реальную длительность), выполняется только при финальном рендере

### Исправленные баги (2026-03-27)
- **B19:** SFX-клипы не отображались после F5 — кэш `sceneIndex` пуст → `__unresolved__` путь → fallback `readSceneIndex(storage)`
- **B20:** Ширина клипа не восстанавливалась после сброса скорости — stale `optimisticResizes` не очищались при обновлении данных из OPFS
...
### Этап 3: Локальная синхронизация структуры книги ✅

**Принцип: исходный файл книги (PDF) НИКОГДА не покидает устройство пользователя.**
Файл хранится как ссылка на локальный путь (`source/book.pdf` внутри ProjectStorage) и читается оттуда при необходимости.
...
- `src/hooks/useBookManager.ts` — принимает `projectStorage?`, записывает структуру локально
- `src/pages/Parser.tsx` — авто-синк при изменении `chapterResults` (после анализа глав)

**Что записывается локально (автоматически):**
...
- `source/book.pdf` — ссылка/копия исходного PDF (при upload, остаётся локально)

**Точки автосохранения в локальное хранилище:**
1. `handleFileSelect` — после загрузки PDF → создание структуры + запись в ProjectStorage
2. `openSavedBook` — при открытии книги → восстановление из локалки (сервер = фоллбек)
...
- Сервер получает данные ТОЛЬКО по явной команде пользователя (кнопка «На сервер»)
...
- **Этап 5:** Кнопка «Сохранить в облако» — сериализация структурных метаданных → Supabase
- **Этап 6:** Кнопка «Загрузить из облака» — восстановление структуры из Supabase в локалку
- **Этап 7:** Чтение структуры из локалки при отсутствии сети (offline-first)

### Ключевые архитектурные решения:
1. **PDF остаётся локальным** — файл книги никогда не загружается на сервер; на сервер уходят только извлечённые текстовые блоки для ИИ-анализа и TTS
...
## Модуль 3.1: Очистка текста и сноски (Content Cleanup)

**Статус:** ✅ Реализовано

### Архитектура
- `src/hooks/useContentCleanup.ts` — чистые функции очистки, хук `applyCleanup`
- `src/hooks/useSelectionCapture.ts` — shared-хук для сохранения текстовой селекции при контекстном меню
- Контекстное меню на карточках сцен в `ChapterDetailPanel`
- Глобальные кнопки в шапке списка сцен (без выделения)

### Действия очистки

| Действие | Тип | Описание |
|----------|-----|----------|
| **Колонтитул** | Контекст-меню | Fuzzy regex по выделению → удаление всех похожих вхождений по всем сценам |
| **Номер страницы** | Контекст-меню | Удаление standalone чисел на отдельных строках |
| **Раздел главы** | Контекст-меню | Разделение сцены в точке выделения (если после — заглавная буква) |
| **Номер сноски** | Контекст-меню | Обёртка выделенного числа в маркер `[сн.→ N]`, проверка наличия тела сноски |
| **Авто-сноски** | Кнопка | Массовый поиск: собирает номера из `[сн. N]` маркеров, ищет голые числа в теле текста, оборачивает в `[сн.→ N]` |
| **Пробелы** | Кнопка | Удаление пробелов перед пунктуацией, добавление после |

### Маркеры сносок
- `[сн. N]...[/сн.]` — тело сноски (из PDF extraction, янтарный бейдж)
- `[сн.→ N]` — ссылка на сноску в теле текста (голубой бейдж)
- Связь по номеру N: ссылка → тело
- Маркеры сохраняются при сегментации в Студии (промпт `segment-scene` инструктирован)
- `PhraseRenderer` рендерит `[сн.→ N]` голубым бейджем

### useSelectionCapture (shared паттерн)
- **Проблема:** браузер сбрасывает `window.getSelection()` при клике на пункт контекстного меню
- **Решение:** хук `capture()` сохраняет селекцию в `useRef` при `onContextMenu`, `consume()` читает и очищает
- **С DOM-оффсетами:** при передаче `containerRef` вычисляет `start/end` через `TreeWalker`
- **Используется:** `ChapterDetailPanel` (Парсер), `EditablePhrase` (Раскадровка Студии)

### ⏳ Запланировано
- **Выбор момента озвучки сноски** в Раскадровке: контекстное меню на `[сн.→ N]` с вариантами «После слова», «В конце фразы», «В конце блока»
- **OCR-артефакты:** AI-коррекция склеенных слов и переносов
...
## Сессия 2026-03-29: AI-роли, пресеты, индикатор перевода

### B23: Дублирование пресетов AI-ролей ✅
**Проблема:** Поле «Имя пресета» при сохранении оставалось пустым, Update-or-Create создавал дубликаты.
**Решение:** Автозаполнение поля имени из `bookTitle` при открытии диалога.

### B24: Пресеты не сохраняли OpenRouter-модели 🔍
**Проблема:** Пайплайн перевода получал модель Lovable AI вместо выбранной OpenRouter → ошибка 402.
**Частичное решение:** Сохранение `currentOverrides` вместо `resolvedModels`; `loadPreset` применяет все модели как overrides.
**Статус:** Требует расследования — текущая гипотеза о фоллбэке несостоятельна (см. TODO.md).

### B25: Индикатор перевода «Сцены: 0/1» ✅
**Проблема:** Счётчик не показывал посегментный прогресс до завершения пайплайна.
**Решение:** Немедленная инициализация `currentStage` при старте `runSingleScene`.
